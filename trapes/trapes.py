    ...
    for cell_folder in os.listdir(path):
        ...
        if ((os.path.exists(full_path)) & (os.path.isdir(full_path))):
            ...
            if not found:
                ...
                reconstruction = curr_folder + '../bin/vdj.alignment'
                if genome == 'hg38':
...
def main():
    ...
    parser.add_argument('-single_cell', 
        help='add if you are only running on a single cell. If so, it will ignore -path and -subpath arguments', 
        action='store_true')
    parser.add_argument('-low_q', 
        help='add if you want to add \"low quality\" reads as input to the reconstruction \
        algorithm', 
        action='store_true')
    parser.add_argument('-one_side', 
        help='add if you want to observe reconstrctuion only from the V side',
        action='store_true')
    parser.add_argument('-path', '-p', '-P', 
        help='The path for the data directory. Assumes that every subdirectory is a single cell', 
        default='')
    parser.add_argument('-sum_f', 
        help='prefix for summary outputs', 
        default='')
    parser.add_argument('-bowtie2', '-bw', '-BW', 
        help='Path to bowtie2. If not used assumes that bowtie2 is in the default path', 
        default='')
    parser.add_argument('-rsem', '-RSEM', 
        help='Path to rsem. If not used assumes that rsem is in the default path', 
        default='')
    parser.add_argument('-strand',
        help='Strand of the right most read in genomic coordinates. Options are: [minus, plus, none]. Defualt is minus', 
        default='minus')
    parser.add_argument('-output', '-out', '-o', '-O', 
        help='output prefix, relative to /path/singleCellFolder',
        required=True)
    parser.add_argument('-bam',
        help='Input bam alignment file, relative to /path/singleCellFolder/ if working on multiple files',
    ...
    parser.add_argument('-bases', '-b', '-B',
        ...
        type=int, default=-10)
    parser.add_argument('-iterations', '-iter', '-i', '-I', 
        help='Number of iterations for the reconstruction algorithm, default is 20', 
        type=int, default=20)
    parser.add_argument('-samtools', 
        help='Path to samtools. If not used assumes that samtools is in the default path',
        default='')
    parser.add_argument('-score', '-sc', '-SC', 
        help='Alignment score threshold. Default is 15', 
        type=int, default=15)
    parser.add_argument('-top', '-t', '-T', 
        help='Take only the top x combination of V and J, based on the sum number of reads that map to both. Default is to take all', 
        type=int, default=-1)
    parser.add_argument('-read_overlap', '-ro', '-readoverlap',
        help='Add a read to list of mapped reads only if it maps at least X bases to the V/J/C segment. Default is 1', 
        type=int, default=1)
    parser.add_argument('-by_exp',
        help='if using the Top option, add this tag if you want to take only two chains from each read count, until top is reached', 
        action='store_true')
    ...
    run_tcr_pipe(args.genome, args.output, args.bam, args.unmapped, args.bases, args.strand,
        ...
        args.read_overlap, args.one_side, args.trim)
